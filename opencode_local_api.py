...
def ask_permission(cmd_or_action):
    ...
    tk.Label(root, text="OpenCode is requesting action:", font=("Arial", 12, "bold"), bg='#1e1e1e', fg='white').pack(pady=(15, 5))
    cmd_text = tk.Text(root, height=3, width=55, bg="#2d2d2d", fg="#d4d4d4", font=("Consolas", 10), bd=0, highlightthickness=1, highlightbackground="#3e3e42")
    ...
    btn_run = tk.Button(btn_frame, text="Allow (Alt+Enter)", command=on_run, bg="#4CAF50", fg="white", font=("Arial", 10, "bold"), width=15, relief=tk.FLAT)
    btn_run.pack(side=tk.LEFT, padx=10)
    btn_cancel = tk.Button(btn_frame, text="Deny (Esc)", command=on_cancel, bg="#f44336", fg="white", font=("Arial", 10, "bold"), width=15, relief=tk.FLAT)
